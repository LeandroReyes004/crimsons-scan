...
pattern = r'// Vista para admin_scan: solo su propio scan\s*if \(ownScanId\)\s*\{.*?return\s*\(.*?\);\s*\}'
new_code = re.sub(pattern, new_ownscan_view, full_code, flags=re.DOTALL)

with io.open('frontend/src/app/admin/page.tsx', 'w', encoding='utf-8') as f:
    f.write(new_code)
print('Done patching SectionRevenue ownScanId!')
