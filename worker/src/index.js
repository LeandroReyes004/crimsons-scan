...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (pathname === '/api/auth/me' && method === 'GET') {
        ...
        if (profile && profile.scan_id) {
          const scanData = await env.DB.prepare('SELECT nombre, contrato_firmado, contrato_version FROM scans WHERE id = ?').bind(profile.scan_id).first();
          profile.scan_nombre = scanData ? scanData.nombre : null;
    ...
    if (pathname.match(/^\/api\/admin\/mangas\/[a-zA-Z0-9-]+\/toggle_hidden$/) && method === 'POST') {
      ...
      if (!caller || (!caller.is_superadmin && caller.rol !== 'admin' && caller.rol !== 'admin_scan')) return err('No autorizado', 403);
    ...
    if (pathname === '/api/mangas' && method === 'POST') {
      ...
      if (pathname === '/api/admin/stats' && method === 'GET') {
        ...
        if (mes && /^\d{4}-\d{2}$/.test(mes)) {
          ...
          filterUploaders = " AND c.fecha_publicacion >= ? AND c.fecha_publicacion < ?";
          paramsUploaders.push(fechaInicio, fechaFin);
        }
      ...
      if (editWebhook && method === 'PUT') {
        ...
        const { webhook_discord, discord_template, telegram_chat_id, telegram_template } = await request.json();
