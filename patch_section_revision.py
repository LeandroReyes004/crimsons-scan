...
with io.open('revision_form_extract_utf8.txt', 'r', encoding='utf-8') as f:
    old_revision_form = f.read().strip()
...
import re
if old_revision_form in full_code:
    full_code = full_code.replace(old_revision_form, new_revision_form)
    with io.open('frontend/src/app/admin/page.tsx', 'w', encoding='utf-8') as f:
        f.write(full_code)
    print("SectionRevision replaced successfully")
else:
    print("old_revision_form NOT FOUND in page.tsx")
    # regex fallback
    full_code = re.sub(r'function SectionRevision.*?(?=\n  // ============================================================\n  //  SECCIÓN: USUARIOS)', new_revision_form + '\n', full_code, flags=re.DOTALL)
    with io.open('frontend/src/app/admin/page.tsx', 'w', encoding='utf-8') as f:
        f.write(full_code)
    print("SectionRevision replaced via regex fallback")
