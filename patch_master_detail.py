...
new_section_scans = """function SectionScans() {
...
# Use regex to match function SectionScans() { ... } up to the next section or the end of the file.
new_code = re.sub(r'function SectionScans\(\)\s*\{.*?(?=\n\s*// ===+|\n\s*function SectionRevenue|\n\s*function SectionUsuarios)', new_section_scans, full_code, flags=re.DOTALL)

with io.open('frontend/src/app/admin/page.tsx', 'w', encoding='utf-8') as f:
    f.write(new_code)
print('Done patching SectionScans!')
