...
# First restore to the working commit to discard the bad patch
subprocess.run(['git', 'checkout', '--', 'frontend/src/app/admin/page.tsx'])
...
end_idx = full_code.find(end_marker)

if start_idx != -1 and end_idx != -1:
    new_code = full_code[:start_idx] + new_ownscan_view + "\n\n" + full_code[end_idx:]
    with io.open('frontend/src/app/admin/page.tsx', 'w', encoding='utf-8') as f:
        f.write(new_code)
    print("Patch applied successfully using exact index replace.")
else:
