  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/chapters/secret/${token}`)
      ...
      .then(d => {
        ...
        if (d.capitulo?.manga_tipo?.toLowerCase() === 'novela') {
          fetch(`${API}/api/chapters/secret/${token}/text`)
            .then(r => r.text())
            .then(t => { if (!cancelled) setNovelText(t); })
            .catch(e => { if (!cancelled) setError("No se pudo cargar el texto"); })
    ...
    <div className="min-h-screen bg-[#111] text-white font-sans pb-32 select-none">
      ...
      <header className={`fixed top-0 inset-x-0 z-40 bg-[#0a0a0c]/90 backdrop-blur-md border-b border-white/5 h-14 px-3 sm:px-6 flex items-center justify-between gap-2 transition-all duration-300 ${!isNavVisible ? '-translate-y-full opacity-0 pointer-events-none' : 'translate-y-0 opacity-100 pointer-events-auto'}`}>
        <div className="flex items-center gap-1">
          ...
          {/* Prev chapter */}
          {(capInfo as any)?.prev_token && (
            <Link href={`/leer/${(capInfo as any).prev_token}`}
          ...
          {/* Next chapter */}
          {(capInfo as any)?.next_token && (
            <Link href={`/leer/${(capInfo as any).next_token}`}
      ...
      <main className={`flex flex-col items-center pt-14 ${readingMode === 'webtoon' ? 'w-full max-w-[900px] mx-auto' : 'max-w-4xl mx-auto px-2'}`}>
        ...
        {/* Final del capítulo */}
        {!loading && !error && (capInfo?.manga_tipo?.toLowerCase() === 'novela' || (pages.length > 0 && readingMode === 'webtoon')) && (
      ...
      {capInfo?.manga_tipo?.toLowerCase() !== 'novela' && pages.length > 0 && (
        <ReaderControls
